...
// ---------------------------------------------------------------------------
// Main export — Gemini Vision → raw OCR text
// ---------------------------------------------------------------------------
 ...
 * @param onStatus  Optional UI status callback
...
export async function extractTextFromImage(
  ...
  console.log(
    ...
    JSON.stringify({ ...requestBody, contents: [{ parts: [{ text: OCR_PROMPT }, { inline_data: { mime_type: mimeType, data: "[BASE64 OMITTED]" } }] }] }, null, 2)
  );

  onStatus?.("Reading bill...");
  console.time("[OCR] Gemini API call");
  ...
  console.timeEnd("[OCR] Gemini API call");

  // ── Log full raw response body BEFORE any processing ─────────────────────
  const rawBody = await response.text();
  console.log("[OCR] HTTP status:", response.status);
  console.log("[OCR] Full Gemini response:", JSON.stringify(JSON.parse(rawBody), null, 2));

  if (!response.ok) {
    throw new Error(`Gemini OCR error (${response.status}): ${rawBody}`);
  }
